...
import {
  ...
  Loader2, AlertCircle, RotateCcw, Square, CheckSquare, GripVertical, FileText, Download,
} from "lucide-react";
...
/* ------------------------------------------------------------------ */
/*  Per-project export — bundle everything for one project into text    */
/*  so it can be handed to Claude for progress / decision-log Q&A.      */
/* ------------------------------------------------------------------ */
const MTYPE_LABEL = {
  tiger: "Tiger Team", innovation: "Innovation", dashboard: "Dashboard",
  trish: "Trish 1:1", others: "기타", manual: "수동",
};
function buildProjectExport(p, notes, transcripts, opts = {}) {
  const includeTx = opts.includeTranscripts !== false;
  const L = [];
  L.push(`# ${p.name}`);
  L.push("");
  L.push(`- 상태: ${p.status || "—"}`);
  if (p.lead && p.lead !== "—") L.push(`- 담당: ${p.lead}`);
  if (p.targetDate && p.targetDate !== "—") L.push(`- 타깃: ${p.targetDate}`);
  if (p.page && p.page !== "—") L.push(`- 위치: ${p.page}${p.location && p.location !== "—" ? ` (${p.location})` : ""}`);
  L.push(`- 내보낸 날짜: ${new Date().toISOString().slice(0, 10)}`);
  L.push("");

  // dated meeting entries, oldest → newest (chronological history)
  const entries = [...(p.entries || [])].sort((a, b) => (a.date || "").localeCompare(b.date || ""));
  if (entries.length) {
    L.push("## 미팅 히스토리 (시간순)");
    L.push("");
    entries.forEach((e) => {
      const mt = MTYPE_LABEL[e.mtype] || e.mtype || "";
      L.push(`### ${e.date || "날짜 미상"}${mt ? ` · ${mt}` : ""}`);
      if (e.summary) L.push(`요약: ${e.summary}`);
      if (e.currentStatus) L.push(`상태: ${e.currentStatus}`);
      if (e.brettFeedback) L.push(`Brett 피드백: ${e.brettFeedback}`);
      if (e.brettQuotes) {
        toBullets(e.brettQuotes).forEach((q) => L.push(`  · Brett: "${q}"`));
      }
      if (e.otherFeedback) L.push(`기타 피드백: ${e.otherFeedback}`);
      L.push("");
    });
  }

  // open action items
  const openSteps = (p.nextSteps || []).filter((s) => !s.done).map((s) => s.text);
  const doneSteps = (p.nextSteps || []).filter((s) => s.done).map((s) => s.text);
  const openWork = (p.myWork || []).filter((s) => !s.done).map((s) => s.text);
  const doneWork = (p.myWork || []).filter((s) => s.done).map((s) => s.text);
  if (openSteps.length || doneSteps.length) {
    L.push("## Next Step");
    openSteps.forEach((t) => L.push(`- [ ] ${t}`));
    doneSteps.forEach((t) => L.push(`- [x] ${t}`));
    L.push("");
  }
  if (openWork.length || doneWork.length) {
    L.push("## My Work");
    openWork.forEach((t) => L.push(`- [ ] ${t}`));
    doneWork.forEach((t) => L.push(`- [x] ${t}`));
    L.push("");
  }

  // linked reference notes
  const linked = (notes || []).filter((n) => n.projectId === p.id)
    .sort((a, b) => (a.date || "").localeCompare(b.date || ""));
  if (linked.length) {
    L.push("## 연결된 노트");
    L.push("");
    linked.forEach((n) => {
      L.push(`### ${n.date || ""} · ${n.title || "제목 없음"}`);
      if (n.summary) L.push(`요약: ${n.summary}`);
      if (n.key) L.push(`핵심: ${n.key}`);
      const fu = (n.followUps || []).map((f) => f.text).filter(Boolean);
      if (fu.length) L.push(`F/U: ${fu.join(" · ")}`);
      L.push("");
    });
  }

  // full transcripts for every date this project touched
  if (includeTx) {
    const dates = new Set();
    entries.forEach((e) => e.date && dates.add(e.date));
    linked.forEach((n) => n.date && dates.add(n.date));
    const txDates = [...dates].sort().filter((d) => transcripts && transcripts[d] && String(transcripts[d]).trim());
    if (txDates.length) {
      L.push("## 미팅 원문 (transcript)");
      L.push("");
      txDates.forEach((d) => {
        L.push(`### ${d} 원문`);
        L.push(String(transcripts[d]).trim());
        L.push("");
      });
    }
  }
...
// A "meeting" is a unique (date + meeting-type) pair — the same day can hold
// e.g. a TT and a 1:1, and each is its own selectable meeting in the dropdown.
const mkey = (date, mtype) => `${date || ""}|${mtype || "manual"}`;

const allMeetingsOf = (projects) => {
  const seen = new Map(); // key -> { key, date, mtype }
  (projects || []).forEach((p) => (p.entries || []).forEach((e) => {
    if (!e.date) return;
    const k = mkey(e.date, e.mtype);
    if (!seen.has(k)) seen.set(k, { key: k, date: e.date, mtype: e.mtype || "manual" });
  }));
  // newest date first; within a date, order by MTYPE_ORDER
  return [...seen.values()].sort((a, b) => {
    if (a.date !== b.date) return a.date < b.date ? 1 : -1;
    return MTYPE_ORDER.indexOf(a.mtype) - MTYPE_ORDER.indexOf(b.mtype);
  });
};

// meeting key -> "MM/DD · TT" label
const meetingLabel = (m) => {
  const d = m.date || "";
  const md = d.length >= 10 ? `${d.slice(5, 7)}/${d.slice(8, 10)}` : d;
  const short = (MTYPE[m.mtype] && MTYPE[m.mtype].short) || "";
  return short ? `${md} · ${short}` : md;
};
...
export default function TigerTeamTracker() {
  ...
  const [selKeys, setSelKeys] = useState(() => new Set()); // empty = latest; else show only these meetings (date|mtype)
  const toggleMeeting = (k) => setSelKeys((prev) => { const n = new Set(prev); n.has(k) ? n.delete(k) : n.add(k); return n; });
  const clearDates = () => setSelKeys(new Set());
  ...
  const allMeetings = allMeetingsOf(projects || []);
  const isHistory = selKeys.size > 0;

  // ── edit / delete a whole meeting (all entries sharing a date|mtype) ──
  const deleteMeeting = (key) => {
    const [d] = key.split("|");
    const next = (projects || []).map((p) => ({
      ...p,
      entries: (p.entries || []).filter((e) => mkey(e.date, e.mtype) !== key),
    }));
    saveProjects(next);
    // drop transcript only if no remaining entry uses that date
    const stillUsed = next.some((p) => (p.entries || []).some((e) => e.date === d));
    if (!stillUsed && transcripts[d]) {
      const t = { ...transcripts }; delete t[d]; saveTranscripts(t);
    }
    // deselect if it was selected
    setSelKeys((prev) => { const n = new Set(prev); n.delete(key); return n; });
  };

  const editMeeting = (oldKey, newDate, newMtype) => {
    const [oldDate] = oldKey.split("|");
    const nd = (newDate && String(newDate).trim()) ? newDate : oldDate;
    const nm = newMtype || oldKey.split("|")[1] || "manual";
    const next = (projects || []).map((p) => ({
      ...p,
      entries: (p.entries || []).map((e) =>
        mkey(e.date, e.mtype) === oldKey ? { ...e, date: nd, mtype: nm } : e),
    }));
    saveProjects(next);
    // move transcript if the date changed
    if (nd !== oldDate && transcripts[oldDate]) {
      const t = { ...transcripts };
      if (!t[nd]) t[nd] = t[oldDate];
      delete t[oldDate];
      saveTranscripts(t);
    }
    // update selection key if it was selected
    setSelKeys((prev) => {
      if (!prev.has(oldKey)) return prev;
      const n = new Set(prev); n.delete(oldKey); n.add(mkey(nd, nm)); return n;
    });
  };

  // entries to render per project given the current view + meeting-type filter
  const entriesFor = (p) => {
    const base = isHistory ? p.entries.filter((e) => selKeys.has(mkey(e.date, e.mtype))) : p.entries;
    const filtered = base.filter((e) => mtypeOn.has(e.mtype));
    return isHistory ? filtered : filtered.slice(0, VISIBLE_ENTRIES);
  };

  // in history view, hide projects with no entry in any selected meeting
  const shownProjects = (projects || []).filter(
    (p) => !isHistory || p.entries.some((e) => selKeys.has(mkey(e.date, e.mtype))));
  ...
  return (
    <div style={{ background: C.paper, color: C.ink }}
      ...
      <header style={{ background: C.paper }}
        className="sticky top-0 z-30 max-w-[1600px] w-full mx-auto px-3 sm:px-10 pt-4 pb-3 shrink-0">
            ...
            <div className="flex items-center gap-2">
              {tab === "projects" ? (
                <>
                  ...
                  <DateFilter allMeetings={allMeetings} selKeys={selKeys}
                    onToggle={toggleMeeting} onClear={clearDates} isHistory={isHistory}
                    transcripts={transcripts} onOpenTx={(d) => setTxDate(d)}
                    onEditMeeting={editMeeting} onDeleteMeeting={deleteMeeting} />
      ...
      <div style={{ borderColor: C.border, background: "#fff" }}
        className="shrink-0 border-t max-w-[1600px] w-full mx-auto px-3 sm:px-10 py-2.5
      ...
      <main className="w-full max-w-[1600px] mx-auto">
        <div style={{ borderColor: C.border }} className="border-y bg-white w-full overflow-x-hidden">
          <table className="border-separate border-spacing-0 text-left w-full table-auto sm:table-fixed">
            ...
            <tbody>
              {items.filter((it) => it.kind === "row").map((it) => (
                <TableRow key={it.p.id} p={it.p} entries={entriesFor(it.p)} readOnly={isHistory}
                  ...
                  onExport={(withTx) => downloadText(
                    `${it.p.id || it.p.name}_${new Date().toISOString().slice(0, 10)}.md`,
                    buildProjectExport(it.p, notes, transcripts, { includeTranscripts: withTx })
                  )}
      ...
      <footer style={{ borderColor: C.border, background: C.paper }}
        className="border-t max-w-[1600px] w-full mx-auto px-3 sm:px-10 py-3">
        {confirmReset ? (
      ...
      {showAdd && (
        <AddMeeting
          ...
          onApply={(updates, mtype, date, transcript) => {
            const entryDate = (date && String(date).trim()) ? date : new Date().toISOString().slice(0, 10);
            const next = [...projects];
            updates.forEach((u) => {
              ...
              const entry = {
                id: uid(), date: entryDate, mtype: mtype || "manual",
            ...
            if (transcript && transcript.trim()) {
              saveTranscripts({ ...transcripts, [entryDate]: transcript });
            }
            setShowAdd(false);
          }}
        />
      ...
      {showAddNote && (
        <AddNote
          ...
          onApply={(newNotes, newTx) => {
            saveNotes([...newNotes, ...notes]);
            if (newTx && Object.keys(newTx).length) {
              saveTranscripts({ ...transcripts, ...newTx });
            }
...
/* ------------------------------------------------------------------ */
function DateFilter({ allMeetings, selKeys, onToggle, onClear, isHistory, transcripts, onOpenTx, onEditMeeting, onDeleteMeeting }) {
  const [open, setOpen] = useState(false);
  const [editKey, setEditKey] = useState(null);   // meeting key being edited
  const [confirmKey, setConfirmKey] = useState(null); // meeting key pending delete
  const [eDate, setEDate] = useState("");
  const [eType, setEType] = useState("manual");
  const label = isHistory ? `${selKeys.size}개 미팅 선택` : `최근 ${VISIBLE_ENTRIES}개 (기본)`;

  const startEdit = (m) => { setEditKey(m.key); setEDate(m.date); setEType(m.mtype); setConfirmKey(null); };
  const saveEdit = () => { onEditMeeting(editKey, eDate, eType); setEditKey(null); };
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => { setOpen(false); setEditKey(null); setConfirmKey(null); }} />
          <div style={{ borderColor: C.border }}
            className="absolute right-0 mt-1 z-50 w-72 max-h-96 overflow-y-auto bg-white rounded-lg border shadow-lg p-1.5">
            ...
            <div style={{ borderColor: C.border }} className="border-t pt-1">
              <div style={{ color: C.faint }} className="text-[10px] font-semibold uppercase tracking-wider px-2.5 py-1">보고 싶은 미팅</div>
              {allMeetings.length === 0 && (
                <div style={{ color: C.faint }} className="text-xs px-2.5 py-2">아직 미팅이 없어요</div>
              )}
              {allMeetings.map((m) => {
                const on = selKeys.has(m.key);
                const hasTx = !!(transcripts && transcripts[m.date] && transcripts[m.date].trim());

                // ── inline edit form ──
                if (editKey === m.key) {
                  return (
                    <div key={m.key} style={{ background: C.violetSoft, borderColor: "#A9D8C8" }}
                      className="rounded-md border p-2 my-1 space-y-2">
                      <div className="flex items-center gap-1.5">
                        <input type="date" value={eDate} onChange={(e) => setEDate(e.target.value)}
                          style={{ borderColor: C.border }}
                          className="flex-1 text-xs rounded border px-1.5 py-1 bg-white" />
                        <select value={eType} onChange={(e) => setEType(e.target.value)}
                          style={{ borderColor: C.border }}
                          className="text-xs rounded border px-1 py-1 bg-white">
                          {MTYPE_ORDER.map((k) => <option key={k} value={k}>{MTYPE[k].short}</option>)}
                        </select>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <button onClick={saveEdit} style={{ background: C.violet }}
                          className="flex-1 text-white text-xs font-medium py-1.5 rounded hover:opacity-90 transition">저장</button>
                        <button onClick={() => setEditKey(null)} style={{ color: C.muted, borderColor: C.border }}
                          className="text-xs px-3 py-1.5 rounded border hover:bg-black/5 transition">취소</button>
                      </div>
                    </div>
                  );
                }
                ...
                if (confirmKey === m.key) {
                  return (
                    <div key={m.key} style={{ background: "#FEF2F2", borderColor: "#FECACA" }}
                      className="rounded-md border p-2 my-1">
                      <div style={{ color: "#B91C1C" }} className="text-[11px] mb-2">
                        “{meetingLabel(m)}” 미팅을 삭제할까요? 이 미팅의 모든 프로젝트 기록이 지워져요.
                      </div>
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => { onDeleteMeeting(m.key); setConfirmKey(null); }}
                          style={{ background: "#E11D48" }}
                          className="flex-1 text-white text-xs font-medium py-1.5 rounded hover:opacity-90 transition">삭제</button>
                        <button onClick={() => setConfirmKey(null)} style={{ color: C.muted, borderColor: C.border }}
                          className="text-xs px-3 py-1.5 rounded border hover:bg-black/5 transition bg-white">취소</button>
                      </div>
                    </div>
                  );
                }
                ...
                // ── normal row ──
                return (
                  <div key={m.key} className="flex items-center gap-0.5 rounded-md hover:bg-black/5">
                    <button onClick={() => onToggle(m.key)}
                      className="flex-1 text-left text-xs px-2.5 py-1.5 inline-flex items-center gap-2 min-w-0">
                      {on ? <CheckSquare size={14} style={{ color: C.violet }} className="shrink-0" />
                          : <Square size={14} style={{ color: C.faint }} className="shrink-0" />}
                      <span style={{ color: on ? C.ink : C.muted }} className="font-medium truncate">{meetingLabel(m)}</span>
                    </button>
                    <button onClick={() => { setOpen(false); onOpenTx(m.date); }}
                      title={hasTx ? "미팅 원문 보기" : "미팅 원문 올리기"} className="shrink-0 px-1.5 py-1.5">
                      <FileText size={13} style={{ color: hasTx ? C.violet : C.faint, opacity: hasTx ? 1 : 0.5 }} />
                    </button>
                    <button onClick={() => startEdit(m)} title="미팅 날짜/종류 수정" className="shrink-0 px-1.5 py-1.5">
                      <PenLine size={13} style={{ color: C.muted }} />
                    </button>
                    <button onClick={() => { setConfirmKey(m.key); setEditKey(null); }} title="미팅 삭제" className="shrink-0 px-1.5 py-1.5">
  ...
  return (
    <div>
      <div style={{ color: C.faint }}
        className="text-[10px] font-bold uppercase tracking-wide mb-1">{label}</div>
      <div>{children}</div>
    </div>
  );
}

function TableRow({ p, entries, onSave, onDelete, onExport, readOnly, dim, collapsed, onToggleCollapse, colW, onColLive, onColCommit, projDragId, onProjDragStart, onProjDragEnd, onProjDrop }) {
  ...
  return (
    <>
        ...
        <td colSpan={TCOLS.length} onClick={onToggleCollapse}
          ...
          <div className="flex items-center justify-between gap-3 px-4 py-2" style={{ opacity: dim ? 0.6 : 1 }}>
            ...
            <div className="flex items-center gap-2 shrink-0" onClick={stop}>
              {/* export this project's full history for Claude Q&A */}
              <div className="relative">
                <button onClick={() => setExportMenu((v) => !v)} title="이 프로젝트 내보내기 (Claude에 제공용)">
                  <Download size={15} style={{ color: exportMenu ? C.violet : C.muted }} />
                </button>
                {exportMenu && (
                  <div style={{ background: C.card, borderColor: C.border }}
                    className="absolute right-0 mt-1 z-20 w-48 rounded-lg border shadow-lg overflow-hidden">
                    <button onClick={() => { setExportMenu(false); onExport(true); }}
                      style={{ color: C.ink }}
                      className="block w-full text-left text-xs px-3 py-2 hover:bg-black/5 transition">
                      원문 포함 (전체)
                    </button>
                    <button onClick={() => { setExportMenu(false); onExport(false); }}
                      style={{ color: C.ink, borderColor: C.border }}
                      className="block w-full text-left text-xs px-3 py-2 border-t hover:bg-black/5 transition">
                      요약만 (원문 제외)
                    </button>
                  </div>
                )}
      ...
      {!collapsed && (
        <tr className="hidden sm:table-row" style={{ opacity: dim ? 0.6 : 1 }}>
          {TCOLS.map((c, i) => (
      ...
      {/* mobile stacked content — one full-width cell with labeled sections */}
      {!collapsed && (
        <tr className="sm:hidden" style={{ opacity: dim ? 0.6 : 1 }}>
          <td colSpan={TCOLS.length} style={border} className="px-4 py-3 border-b">
            <div className="space-y-3">
              <MobileSection label={TCOLS[0]}>
                <EntryStack entries={entries} field="currentStatus" />
              </MobileSection>
              <MobileSection label={TCOLS[1]}>
                <Checklist items={p.nextSteps || []} readOnly={readOnly} onChange={setSteps}
                  allowAdd={false} placeholder="F/U 추가…" emptyHint="—" />
              </MobileSection>
              <MobileSection label={TCOLS[2]}>
                <EntryStack entries={entries} field="brett" />
              </MobileSection>
              <MobileSection label={TCOLS[3]}>
                <EntryStack entries={entries} field="otherFeedback" />
              </MobileSection>
              <MobileSection label={TCOLS[4]}>
                <Checklist items={p.myWork || []} readOnly={readOnly} onChange={setWork}
                  placeholder="할 일 추가…" emptyHint="—" />
              </MobileSection>
            </div>
...
/* ------------------------------------------------------------------ */
function NotesView({ notes, projects, transcripts, onOpenTx, onDelete, onToggleFU }) {
  const [filter, setFilter] = useState("all"); // "all" | "none" | projectId
  const projName = (id) => (projects.find((p) => p.id === id) || {}).name;
  const hasTx = (d) => !!(transcripts && transcripts[d] && String(transcripts[d]).trim());
  ...
  return (
    <main className="w-full">
      <div className="max-w-[1100px] w-full mx-auto px-3 sm:px-10 py-5">
        {/* filter chips */}
          ...
          <div className="space-y-3">
            {shown.map((n) => (
              <NoteCard key={n.id} n={n} projName={projName}
                hasTx={hasTx(n.date)} onOpenTx={() => onOpenTx(n.date)}
...
function NoteCard({ n, projName, hasTx, onOpenTx, onDelete, onToggleFU }) {
  const [confirmDel, setConfirmDel] = useState(false);
    ...
    <div style={{ background: C.card, borderColor: C.border }}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-2 flex-wrap min-w-0">
          ...
          {hasTx && (
            <button onClick={onOpenTx} title="미팅 원문 보기"
              style={{ color: C.violet, borderColor: "#A9D8C8" }}
              className="flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full border hover:bg-black/5 transition">
              <FileText size={11} /> 원문
            </button>
          )}
        </div>
  ...
  "notes": [
    {
      ...
      "transcript": "미팅 한글 번역본 전문 (더듬·반복만 제거하고 내용은 그대로). 원문 아카이브에 date 기준으로 저장됨. 없으면 생략",
...
규칙: 참고성 미팅을 Notes로 정리하는 거야. 프로젝트 진척이 아니라 배경/참고 정보. 한국어로 간결하게. projectId는 명확히 관련될 때만. transcript는 녹취 원문을 주고 보관을 원하면 포함(길어도 자르지 말고 전체). 이 파일 하나를 Notes에 올리면 노트와 transcript가 같이 저장돼.
...
function AddNote({ projects, onClose, onApply }) {
  ...
  const build = () => {
    const builtNotes = preview.map((r) => ({
      id: uid(),
      title: r.title || "제목 없음",
    ...
    // collect any transcripts carried on the note rows, keyed by date
    const builtTx = {};
    preview.forEach((r) => {
      const d = r.date || new Date().toISOString().slice(0, 10);
      if (typeof r.transcript === "string" && r.transcript.trim()) builtTx[d] = r.transcript;
    });
    return { notes: builtNotes, transcripts: builtTx };
  };
          ...
          {!preview ? (
            ...
            <>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => { const b = build(); onApply(b.notes, b.transcripts); }}
                  style={{ background: C.violet }}
