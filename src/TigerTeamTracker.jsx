...
export default function TigerTeamTracker() {
  ...
  // delete a whole meeting (a date): remove its entries from every project + its transcript
  const deleteMeeting = (date) => {
    const nextP = (projects || []).map((p) => ({
      ...p, entries: (p.entries || []).filter((e) => e.date !== date),
    }));
    saveProjects(nextP);
    if (transcripts[date]) { const t = { ...transcripts }; delete t[date]; saveTranscripts(t); }
    setSelDates((prev) => { const n = new Set(prev); n.delete(date); return n; });
    setEditMtgDate(null);
  };
    ...
    <div style={{ background: C.paper, color: C.ink }}
        ...
        <div className="flex items-end justify-between gap-4 flex-wrap">
          <div>
            <div style={{ color: C.violet, letterSpacing: "0.2em" }}
              ...
              Project Log
              <span style={{ color: C.faint, letterSpacing: "normal" }} className="ml-2 font-normal normal-case">build 09-01·report</span>
            </div>
        ...
        <AddMeeting
          ...
          onApply={(updates, mtype, date, transcript, reports) => {
            ...
            if (Array.isArray(reports) && reports.length) {
              saveNotes([...reports.map(buildNote), ...notes]);
            }
            setShowAdd(false);
          }}
        />
      ...
      {editMtgDate && (
        <MeetingEditModal
          ...
          onDelete={() => deleteMeeting(editMtgDate)}
        />
      )}
    </div>
  );
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="flex items-center justify-between pt-1">
            {confirmDel ? (
              <div className="flex items-center gap-2">
                <span style={{ color: "#B01238" }} className="text-xs">이 미팅을 모든 프로젝트에서 삭제할까요?</span>
                <button onClick={onDelete} style={{ background: "#E11D48" }} className="text-white text-xs font-medium px-2.5 py-1.5 rounded-lg">삭제</button>
