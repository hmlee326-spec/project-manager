...
{
  "meetingType": "tiger | innovation | dashboard | trish | manual",
  "date": "YYYY-MM-DD",
  "updates": [
    {
      ...
      "followUps": "내가 남한테 받아야/후속할 것. 각 항목 'F/U 사람: 내용'. ' · '로 구분",
          ...
          {!preview && (
            <>
              <div style={{ background: C.violetSoft, color: C.violet }}
                ...
                받은 JSON을 아래에 붙여넣거나 <code>.json</code> 파일을 올리면 돼요.
              ...
              <div>
                <label style={{ color: C.faint }} className="text-[10px] font-semibold uppercase tracking-wider">JSON 붙여넣기</label>
                <textarea value={text} onChange={(e) => { setText(e.target.value); setFileName(""); }}
                  rows={9} placeholder='{ "meetingType": "...", "date": "...", "updates": [ ... ] }'
                  style={{ borderColor: C.border }}
                  className="w-full mt-1 text-[12px] font-mono leading-relaxed rounded-lg border px-3 py-2.5 bg-white resize-y focus:outline-none focus:ring-2 focus:ring-teal-200" />
              </div>
          ...
          {preview && (
            <>
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => onApply(preview, effMtype, effDate)}
