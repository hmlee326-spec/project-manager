...
export default function TigerTeamTracker() {
  ...
  const [mnotes, setMnotes] = useState([]); // meeting reports (Meeting notes tab)
  const [tab, setTab] = useState("projects"); // "projects" | "notes" | "meetings"
  const [showAdd, setShowAdd] = useState(false);
  const [showAddNote, setShowAddNote] = useState(false);
  const [showAddMnote, setShowAddMnote] = useState(false);
  ...
  useEffect(() => {
    (async () => {
      ...
      const mz = await store.get(K_MNOTES);
      if (Array.isArray(mz)) setMnotes(mz);
  ...
  const saveMnotes = (n) => { setMnotes(n); store.set(K_MNOTES, n); };
  // build a note/report object from a raw JSON note (shared by AddNote and meeting-report import)
  const buildNote = (r) => ({
    id: uid(),
    title: r.title || "제목 없음",
    ...
    <div style={{ background: C.paper, color: C.ink }}
      ...
      <header style={{ background: C.paper }}
        ...
        <div className="flex items-end justify-between gap-4 flex-wrap">
          <div>
            <div style={{ color: C.violet, letterSpacing: "0.2em" }}
              ...
              Project Log
              <span style={{ color: C.faint, letterSpacing: "normal" }} className="ml-2 font-normal normal-case">build 09-01·mobile</span>
            </div>
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="flex items-center gap-2">
              {tab === "projects" ? (
                ...
                <button onClick={() => tab === "meetings" ? setShowAddMnote(true) : setShowAddNote(true)}
                  style={{ background: C.violet }}
                  className="text-white text-sm font-medium px-3.5 py-2 rounded-lg
                    inline-flex items-center gap-1.5 hover:opacity-90 transition">
                  <Plus size={16} /> {tab === "meetings" ? "미팅 리포트 추가" : "노트 추가"}
                </button>
              )}
            </div>
          </div>
        ...
        <div style={{ borderColor: C.border }} className="flex items-center gap-1 border-b mt-4 -mb-4">
          {[["projects", "Projects"], ["notes", "Notes"], ["meetings", "Meeting notes"]].map(([k, label]) => (
            <button key={k} onClick={() => setTab(k)}
              ...
              {k === "meetings" && mnotes.length > 0 ? ` (${mnotes.length})` : ""}
            </button>
          ))}
        </div>
      </header>
      ...
      {tab === "meetings" && (
        <NotesView notes={mnotes} projects={projects}
          emptyHint="아직 미팅 리포트가 없어요. 미팅 JSON을 “미팅 추가”로 올리면 리포트가 여기 쌓이고, “미팅 리포트 추가”로도 올릴 수 있어요."
          onDelete={(id) => saveMnotes(mnotes.filter((n) => n.id !== id))}
          onToggleFU={(noteId, fuId) => saveMnotes(mnotes.map((n) =>
            n.id === noteId
              ? { ...n, followUps: (n.followUps || []).map((f) => f.id === fuId ? { ...f, done: !f.done } : f) }
              : n))}
        />
        ...
        <AddMeeting
          ...
          onApply={(updates, mtype, date, transcript, reports) => {
            ...
            if (Array.isArray(reports) && reports.length) {
              saveMnotes([...reports.map(buildNote), ...mnotes]);
            }
            setShowAdd(false);
          }}
        />
      ...
      {showAddMnote && (
        <AddNote
          projects={projects}
          meeting
          onClose={() => setShowAddMnote(false)}
          onApply={(newNotes) => {
            saveMnotes([...newNotes, ...mnotes]);
            setShowAddMnote(false);
          }}
        />
      )}
...
function AddNote({ projects, onClose, onApply, meeting }) {
  const [text, setText] = useState("");
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6"
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div className="flex items-center justify-between px-5 py-4 sticky top-0"
          ...
          <h3 className="text-base font-semibold">{meeting ? "미팅 리포트 추가 (JSON)" : "노트 추가 (JSON)"}</h3>
          <IconBtn onClick={onClose} title="닫기"><X size={16} style={{ color: C.muted }} /></IconBtn>
        </div>
