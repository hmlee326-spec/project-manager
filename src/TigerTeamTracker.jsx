...
// date -> "MM/DD · TT" (uses the most common meeting type recorded on that date)
const dateLabelsOf = (projects, notes) => {
  const byDate = {}; // date -> { mtype: count }
  (projects || []).forEach((p) => (p.entries || []).forEach((e) => {
    if (!e.date) return;
    byDate[e.date] = byDate[e.date] || {};
    const mt = e.mtype || "others";
    byDate[e.date][mt] = (byDate[e.date][mt] || 0) + 1;
  }));
  (notes || []).forEach((n) => {
    if (!n.date) return;
    byDate[n.date] = byDate[n.date] || {};
    const mt = n.mtype || "others";
    byDate[n.date][mt] = (byDate[n.date][mt] || 0) + 1;
  });
  const out = {};
  Object.keys(byDate).forEach((d) => {
    ...
    const short = (MTYPE[top] && MTYPE[top].short) || "";
    out[d] = short ? `${md} · ${short}` : md;
  });
  return out;
};
...
export default function TigerTeamTracker() {
  ...
  const dateLabels = dateLabelsOf(projects || [], notes || []);
            ...
            <div className="flex items-center gap-2">
              {tab === "projects" ? (
                <>
                  ...
                  <DateFilter allDates={allDates} selDates={selDates} dateLabels={dateLabels}
...
/* ------------------------------------------------------------------ */
function DateFilter({ allDates, selDates, dateLabels, onToggle, onClear, isHistory, transcripts, onOpenTx }) {
                ...
                return (
                  <div key={d} className="flex items-center gap-1 rounded-md hover:bg-black/5">
                    <button onClick={() => onToggle(d)}
                      ...
                      <span style={{ color: on ? C.ink : C.muted }} className="font-medium">{(dateLabels && dateLabels[d]) || d}</span>
                    </button>
