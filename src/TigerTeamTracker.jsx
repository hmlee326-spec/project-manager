...
export default function TigerTeamTracker() {
  ...
  return (
    <div style={{ background: C.paper, color: C.ink }}
      ...
      {/* Header */}
      <header className="max-w-[1600px] w-full mx-auto px-6 sm:px-10 pt-4 pb-3 shrink-0">
...
/* ------------------------------------------------------------------ */
function DateFilter({ allDates, selDates, onToggle, onClear, isHistory }) {
      ...
      {open && (
        <>
          ...
          <div style={{ borderColor: C.border }}
            className="absolute right-0 mt-1 z-50 w-56 max-h-80 overflow-y-auto bg-white rounded-lg border shadow-lg p-1.5">
              ...
              {allDates.map((d) => {
                ...
                return (
                  <button key={d} onClick={() => onToggle(d)}
                    className="w-full text-left text-xs px-2.5 py-1.5 rounded-md hover:bg-black/5 transition inline-flex items-center gap-2">
