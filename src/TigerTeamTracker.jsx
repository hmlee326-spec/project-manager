...
const K_MNOTES = "tigerteam:v2:meetingnotes";
const K_ORDER = "tigerteam:v2:order";
const K_TRANSCRIPTS = "tigerteam:v2:transcripts";
const K_EXCLUDED = "tigerteam:v2:excluded"; // projects deleted on purpose — not auto-recreated by uploads
...
// merge incoming items into a list, skipping any whose text matches an existing UNCHECKED item
// (done items don't block re-adding). Also dedupes within the incoming batch.
const mergeItems = (existing, incoming) => {
  const seen = new Set((existing || []).filter((i) => !i.done).map((i) => normText(i.text)));
  const out = [...(existing || [])];
  (incoming || []).forEach((it) => {
...
// remove unchecked items in a list that came from a given meeting (sourceDate + sourceType)
const purgeSourced = (list, date, mtype) =>
  (list || []).filter((i) => i.done || !(i.sourceDate === date && i.sourceType === mtype));

// one-time cleanup: drop duplicate UNCHECKED items (by normalized text) within a list; keep done items
const dedupeList = (list) => {
  const seen = new Set(); const out = [];
  (list || []).forEach((i) => {
    if (i.done) { out.push(i); return; }
...
// { "YYYY-MM-DD": ["tiger","trish", ...] } — meeting types seen on each date, in MTYPE order
const mtypesByDate = (projects) => {
  const m = {};
  (projects || []).forEach((p) => p.entries.forEach((e) => {
    if (!e.date) return;
    (m[e.date] || (m[e.date] = new Set())).add(e.mtype);
  }));
  const out = {};
  Object.keys(m).forEach((d) => { out[d] = MTYPE_ORDER.filter((k) => m[d].has(k)); });
  return out;
};
...
export default function TigerTeamTracker() {
  ...
  const [notes, setNotes] = useState([]); // 미팅리포트 (unified: former Notes + Meeting notes)
  const [tab, setTab] = useState("projects"); // "projects" | "reports"
  ...
  const [excluded, setExcluded] = useState([]); // [{ id, name }] — deleted projects, skipped on upload
  const [showExcluded, setShowExcluded] = useState(false);
  const [txDate, setTxDate] = useState(null); // date whose transcript modal is open
  const [editMtgDate, setEditMtgDate] = useState(null); // date whose meeting edit modal is open
  ...
  useEffect(() => {
    (async () => {
      ...
      // one-time cleanup: drop duplicate UNCHECKED items in Next Step / My Work (done items kept)
      {
        let changed = false;
        const cleaned = p.map((x) => {
          const ns = dedupeList(x.nextSteps), mw = dedupeList(x.myWork);
          if (ns.length !== (x.nextSteps || []).length || mw.length !== (x.myWork || []).length) changed = true;
          return { ...x, nextSteps: ns, myWork: mw };
        });
        if (changed) { p = cleaned; await store.set(K_PROJECTS, p); }
      }
      setProjects(p);
      ...
      const mz = await store.get(K_MNOTES); // legacy "Meeting notes" tab — merge once into the unified list
      const base = Array.isArray(nz) ? nz : [];
      const legacy = Array.isArray(mz) ? mz : [];
      if (legacy.length) {
        const seen = new Set(base.map((n) => n.id));
        const merged = [...base, ...legacy.filter((n) => !seen.has(n.id))]
          .sort((a, b) => (b.date || "").localeCompare(a.date || ""));
        setNotes(merged); store.set(K_NOTES, merged); store.set(K_MNOTES, []);
      } else {
        setNotes(base);
      }
      ...
      const ex = await store.get(K_EXCLUDED);
      if (Array.isArray(ex)) setExcluded(ex);
    })();
  }, []);

  const saveTranscripts = (t) => { setTranscripts(t); store.set(K_TRANSCRIPTS, t); };
  const saveExcluded = (e) => { setExcluded(e); store.set(K_EXCLUDED, e); };

  // is an incoming update row one of the deliberately-deleted projects?
  const isExcluded = (u) => excluded.some((x) =>
    (u.id && x.id === u.id) || (u.name && x.name && x.name.toLowerCase() === u.name.toLowerCase()));

  // delete a project AND remember it so uploads don't silently recreate it
  const deleteProject = (p) => {
    saveProjects((projects || []).filter((x) => x.id !== p.id));
    if (!excluded.some((x) => x.id === p.id)) saveExcluded([...excluded, { id: p.id, name: p.name }]);
  };
  // allow a project to come back (removes it from the exclude list)
  const restoreExcluded = (id) => saveExcluded(excluded.filter((x) => x.id !== id));

  const saveProjects = (p) => { setProjects(p); store.set(K_PROJECTS, p); };
  const saveNotes = (n) => { setNotes(n); store.set(K_NOTES, n); };
  // build a 미팅리포트 object from a raw JSON note (shared by AddNote and meeting-JSON import)
  const buildNote = (r) => ({
    id: uid(),
    title: r.title || "제목 없음",
  ...
  // merge one project's content into another, then remove the source
  const mergeProject = (sourceId, targetId) => {
    const src = (projects || []).find((p) => p.id === sourceId);
    const tgt = (projects || []).find((p) => p.id === targetId);
    if (!src || !tgt || sourceId === targetId) return;
    const merged = {
      ...tgt,
      entries: [...(tgt.entries || []), ...(src.entries || [])],
      nextSteps: [...(tgt.nextSteps || []), ...(src.nextSteps || [])],
      myWork: [...(tgt.myWork || []), ...(src.myWork || [])],
    };
    saveProjects((projects || []).map((p) => p.id === targetId ? merged : p).filter((p) => p.id !== sourceId));
  };

  // delete one meeting = (date, meetingType):
  //  - remove its dated entries from every project
  //  - cascade: remove unchecked Next Step / My Work items that came from it (done items are kept)
  //  - drop the transcript only if nothing else remains on that date
  const deleteMeeting = (date, mt) => {
    const nextP = (projects || []).map((p) => ({
      ...p,
      entries: (p.entries || []).filter((e) => !(e.date === date && (!mt || e.mtype === mt))),
      nextSteps: mt ? purgeSourced(p.nextSteps, date, mt) : (p.nextSteps || []),
      myWork: mt ? purgeSourced(p.myWork, date, mt) : (p.myWork || []),
    }));
    saveProjects(nextP);
    const stillOnDate = nextP.some((p) => (p.entries || []).some((e) => e.date === date));
    if (!stillOnDate && transcripts[date]) { const t = { ...transcripts }; delete t[date]; saveTranscripts(t); }
    if (!stillOnDate) setSelDates((prev) => { const n = new Set(prev); n.delete(date); return n; });
    setEditMtgDate(null);
  };

  // edit a meeting's date and/or type: applies to every entry on that date, across all projects;
  // item source tags follow so cascade delete / upsert keep working
  const applyMeetingEdit = (oldDate, newDate, newMtype) => {
    const retag = (list) => (list || []).map((i) =>
      i.sourceDate === oldDate ? { ...i, sourceDate: newDate, sourceType: newMtype || i.sourceType } : i);
    const nextP = (projects || []).map((p) => ({
      ...p,
      entries: (p.entries || []).map((e) =>
        e.date === oldDate ? { ...e, date: newDate, mtype: newMtype || e.mtype } : e),
      nextSteps: retag(p.nextSteps),
      myWork: retag(p.myWork),
    }));
    saveProjects(nextP);
    // move the report(s) for that meeting too
    setNotes((prev) => {
      const n = prev.map((r) => r.date === oldDate ? { ...r, date: newDate, mtype: newMtype || r.mtype } : r);
      store.set(K_NOTES, n); return n;
    });
    if (oldDate !== newDate && transcripts[oldDate]) {
      const t = { ...transcripts }; t[newDate] = t[oldDate]; delete t[oldDate]; saveTranscripts(t);
    }
    if (oldDate !== newDate) {
      setSelDates((prev) => { const n = new Set(prev); if (n.has(oldDate)) { n.delete(oldDate); n.add(newDate); } return n; });
    }
    setEditMtgDate(null);
  };

  const allDates = allDatesOf(projects || []);
  const dateTypes = mtypesByDate(projects || []);
  ...
  const entriesFor = (p) => {
    ...
    if (isHistory) return byDateDesc;
    // default view: only the LATEST meeting of each type (a newer Tiger Team pushes out an older one),
    // then keep up to VISIBLE_ENTRIES most recent
    const seen = new Set(); const out = [];
    for (const e of byDateDesc) {
      if (seen.has(e.mtype)) continue;
      seen.add(e.mtype); out.push(e);
      if (out.length >= VISIBLE_ENTRIES) break;
    }
    return out;
  };
    ...
    <div style={{ background: C.paper, color: C.ink }}
      ...
      <header style={{ background: C.paper }}
        ...
        <div className="flex items-end justify-between gap-4 flex-wrap">
          <div>
            <div style={{ color: C.violet, letterSpacing: "0.2em" }}
              ...
              Project Log
              <span style={{ color: C.faint, letterSpacing: "normal" }} className="ml-2 font-normal normal-case">build 09-01·exclude</span>
            </div>
        ...
        <div style={{ borderColor: C.border }} className="flex items-center gap-1 border-b mt-4 -mb-4">
          {[["projects", "Projects"], ["reports", "미팅리포트"]].map(([k, label]) => (
            <button key={k} onClick={() => setTab(k)}
              ...
              {label}
              {k === "reports" && notes.length > 0 ? ` (${notes.length})` : ""}
            </button>
          ))}
        </div>
      </header>
      ...
      <main className="w-full max-w-[1600px] mx-auto">
        <div style={{ borderColor: C.border }} className="border-y bg-white w-full">
          <table className="border-separate border-spacing-0 text-left w-full table-fixed block md:table">
            <colgroup>
            ...
            <tbody className="block md:table-row-group">
              {items.filter((it) => it.kind === "row").map((it) => (
                <TableRow key={it.p.id} p={it.p} entries={entriesFor(it.p)} readOnly={isHistory}
                  ...
                  onMerge={(targetId) => mergeProject(it.p.id, targetId)}
      ...
      {tab === "reports" && (
        <NotesView notes={notes} projects={projects}
          emptyHint="아직 미팅리포트가 없어요. 미팅 JSON을 “미팅 추가”로 올리면 리포트가 여기 쌓이고, “미팅리포트 추가”로 단독 리포트도 올릴 수 있어요."
      ...
      <footer style={{ borderColor: C.border, background: C.paper }}
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            {excluded.length > 0 && (
              <button onClick={() => setShowExcluded((v) => !v)}
                style={{ color: C.faint }}
                className="text-xs inline-flex items-center gap-1.5 hover:opacity-70 transition">
                <Trash2 size={13} /> 제외된 프로젝트 {excluded.length}
        ...
        {showExcluded && excluded.length > 0 && (
          <div style={{ borderColor: C.border, background: C.card }} className="mt-2 rounded-lg border p-3">
            <div style={{ color: C.faint }} className="text-[11px] mb-2">
              삭제한 프로젝트예요. 미팅 JSON에 다시 나와도 자동으로 안 만들어져요. 되살리면 다음 업로드부터 다시 추적돼요.
            </div>
            <ul className="space-y-1">
              {excluded.map((x) => (
                <li key={x.id} className="flex items-center justify-between gap-3 text-xs">
                  <span style={{ color: C.ink }}>{x.name} <span style={{ color: C.faint }} className="font-mono">({x.id})</span></span>
                  <button onClick={() => restoreExcluded(x.id)}
                    style={{ background: C.violetSoft, color: C.violet }}
                    className="font-medium px-2 py-1 rounded-md hover:opacity-80">되살리기</button>
                </li>
              ))}
            </ul>
          </div>
      ...
      {showAdd && (
        <AddMeeting
          ...
          onApply={(updatesIn, mtype, date, transcript, reports, revive) => {
            // excluded (deliberately deleted) projects are skipped unless the user revived them in the preview
            const reviveSet = new Set(revive || []);
            const updates = updatesIn.filter((u, i) => !isExcluded(u) || reviveSet.has(i));
            if (reviveSet.size) {
              const revivedRows = updatesIn.filter((u, i) => reviveSet.has(i));
              saveExcluded(excluded.filter((x) => !revivedRows.some((u) =>
                (u.id && x.id === u.id) || (u.name && x.name && x.name.toLowerCase() === u.name.toLowerCase()))));
            }
            const mt = mtype || "manual";
            // 1) UPSERT: if this (date, meetingType) was uploaded before, remove it first
            //    - drop its dated entries from every project
            //    - cascade: drop unchecked items that came from this meeting (done items are kept)
            let next = (projects || []).map((p) => ({
              ...p,
              entries: (p.entries || []).filter((e) => !(e.date === date && e.mtype === mt)),
              nextSteps: purgeSourced(p.nextSteps, date, mt),
              myWork: purgeSourced(p.myWork, date, mt),
            }));

            // 2) insert (source-tagged, deduped against existing unchecked items)
            const tag = (items) => items.map((i) => ({ ...i, sourceDate: date, sourceType: mt }));
            updates.forEach((u) => {
              const idx = next.findIndex(
                (x) => x.id === u.id || x.name.toLowerCase() === (u.name || "").toLowerCase()
              );
              const entry = {
                id: uid(), date: date || "", mtype: mt,
              ...
              const newSteps = tag(toItems(toBullets(u.followUps)));
              const newTodos = tag(toItems(toBullets(u.nextWork)));
              if (idx >= 0) {
                const cur = next[idx];
                next[idx] = {
                  ...cur,
                  ...
                  nextSteps: mergeItems(cur.nextSteps, newSteps),
                  myWork: mergeItems(cur.myWork, newTodos),
                };
              } else if (u.name) {
                next.push({
                  id: "new-" + uid(),
                  ...
                  entries: [entry], nextSteps: mergeItems([], newSteps), myWork: mergeItems([], newTodos),
                });
              }
            });
            saveProjects(next);

            // 3) transcript: keyed by date → overwrite
            if (transcript && transcript.trim() && date) {
              saveTranscripts({ ...transcripts, [date]: transcript });
            }
            // 4) 미팅리포트: replace any report for the same (date, meetingType), then add
            if (Array.isArray(reports) && reports.length) {
              const kept = notes.filter((n) => !(n.date === date && n.mtype === mt));
              saveNotes([...reports.map(buildNote), ...kept]);
            }
            setShowAdd(false);
          }}
        />
      ...
      {editMtgDate && (
        <MeetingEditModal
          date={editMtgDate}
          types={dateTypes[editMtgDate] || []}
          onClose={() => setEditMtgDate(null)}
          onSave={(newDate, newMtype) => applyMeetingEdit(editMtgDate, newDate, newMtype)}
          onDelete={(mt) => deleteMeeting(editMtgDate, mt)}
        />
      )}
    </div>
  );
...
/* ------------------------------------------------------------------ */
function DateFilter({ allDates, selDates, dateTypes, onToggle, onClear, isHistory, transcripts, onOpenTx, onEditMeeting }) {
      ...
      {open && (
        <>
          ...
          <div style={{ borderColor: C.border }}
            className="absolute left-0 md:left-auto md:right-0 mt-1 z-50 max-h-80 overflow-y-auto bg-white rounded-lg border shadow-lg p-1.5"
            style={{ borderColor: C.border, width: "min(22rem, calc(100vw - 2rem))" }}>
                  ...
                  <div key={d} className="flex items-center gap-1 rounded-md hover:bg-black/5">
                    <button onClick={() => onToggle(d)}
                      ...
                      <span className="inline-flex items-center gap-1">
                        {(dateTypes[d] || []).map((k) => (
                          <span key={k} style={{ background: MTYPE[k].bg, color: MTYPE[k].fg }}
                            className="text-[9px] font-bold px-1.5 py-0.5 rounded">{MTYPE[k].short}</span>
                        ))}
                      </span>
                    </button>
                    <button onClick={() => { setOpen(false); onEditMeeting(d); }}
                      title="미팅 날짜·종류 편집 / 삭제" className="shrink-0 px-1.5 py-1.5">
                      <PenLine size={13} style={{ color: C.faint }} />
                    </button>
...
function MeetingEditModal({ date, types, onClose, onSave, onDelete }) {
  const [newDate, setNewDate] = useState(date);
  const [mtype, setMtype] = useState(types[0] || "tiger");
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-6"
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        className="w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl border shadow-xl">
        <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: `1px solid ${C.border}` }}>
          <h3 className="text-base font-semibold">미팅 편집 · <span className="font-mono text-sm">{date}</span></h3>
          <IconBtn onClick={onClose} title="닫기"><X size={16} style={{ color: C.muted }} /></IconBtn>
        </div>

        <div className="p-5 space-y-4">
          <div style={{ background: C.violetSoft, color: C.violet }} className="text-[12px] leading-relaxed rounded-lg px-3 py-2.5">
            이 날짜의 미팅 기록에 대한 변경은 <b>모든 프로젝트</b>에 함께 적용돼요{multi ? " (이 날짜에 여러 종류가 있어요 — 종류를 바꾸면 모두 하나로 통일됩니다)" : ""}.
          </div>
          ...
          <div>
            ...
            <input type="date" value={newDate} onChange={(e) => setNewDate(e.target.value)}
              ...
              className="block mt-1 text-sm rounded-lg border px-2.5 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-teal-200" />
          </div>

          <div>
            <label style={{ color: C.faint }} className="text-[10px] font-semibold uppercase tracking-wider">종류</label>
            <div className="flex gap-1.5 flex-wrap mt-1.5">
              {MTYPE_ORDER.filter((k) => k !== "manual").map((k) => (
                <button key={k} onClick={() => setMtype(k)}
                  style={{ background: mtype === k ? MTYPE[k].bg : "#FFF", color: mtype === k ? MTYPE[k].fg : C.faint,
                    borderColor: mtype === k ? MTYPE[k].fg : C.border }}
                  className="text-xs font-semibold px-3 py-1.5 rounded-lg border transition">{MTYPE[k].label}</button>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-1">
            {confirmDel ? (
              <div className="flex items-center gap-2">
                <span style={{ color: "#B01238" }} className="text-xs">
                  {date} · {MTYPE[mtype] ? MTYPE[mtype].short : mtype} 미팅을 모든 프로젝트에서 삭제할까요? (이 미팅에서 온 미완료 항목도 함께 삭제, 완료 항목은 유지)
                </span>
                <button onClick={() => onDelete(mtype)} style={{ background: "#E11D48" }} className="text-white text-xs font-medium px-2.5 py-1.5 rounded-lg">삭제</button>
                <button onClick={() => setConfirmDel(false)} style={{ color: C.muted }} className="text-xs px-2 py-1.5">취소</button>
              </div>
            ) : (
              <button onClick={() => setConfirmDel(true)}
                style={{ color: "#B01238" }} className="text-xs font-medium inline-flex items-center gap-1 px-2 py-1.5 rounded-lg hover:bg-black/5">
                <Trash2 size={13} /> 미팅 삭제
              </button>
            )}
            <button onClick={() => onSave(newDate, mtype)}
              style={{ background: C.violet }} className="text-white text-sm font-medium px-4 py-2 rounded-lg hover:opacity-90 transition">저장</button>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <tr className="block md:table-row">
      <td colSpan={TCOLS.length}
        ...
        className="border-b border-t block md:table-cell">
...
/* Section label shown only on narrow screens (columns stack vertically there) */
function MobileLabel({ children, onClick, hint }) {
  return (
    <div className="md:hidden flex items-center justify-between mb-1.5">
      <span style={{ color: C.faint }} className="text-[10px] font-bold uppercase tracking-wide">{children}</span>
      {onClick && (
        <button onClick={onClick} style={{ color: C.violet }} className="text-[10px] font-medium">{hint}</button>
      )}
    </div>
  );
}
...
function TableRow({ p, entries, onSave, onDelete, readOnly, dim, collapsed, onToggleCollapse, colW, onColLive, onColCommit, projectsAll, onMerge, projDragId, onProjDragStart, onProjDragEnd, onProjDrop }) {
  ...
  const [showDoneWork, setShowDoneWork] = useState(false);
  useEffect(() => { setDraft(p); }, [p]);

  const td = "px-3 py-3 border-b align-top block md:table-cell";
    ...
    <>
      ...
      <tr className="block md:table-row">
        <td colSpan={TCOLS.length} onClick={onToggleCollapse}
          ...
          className="border-b border-t cursor-pointer block md:table-cell">
      ...
      {!collapsed && (
        <tr style={{ opacity: dim ? 0.6 : 1 }} className="hidden md:table-row">
          {TCOLS.map((c, i) => {
            const isSteps = i === 1, isWork = i === 4;
            const doneN = isSteps ? (p.nextSteps || []).filter((x) => x.done).length
                        : isWork ? (p.myWork || []).filter((x) => x.done).length : 0;
            const showing = isSteps ? showDoneSteps : isWork ? showDoneWork : false;
            const toggleDone = () => { if (isSteps) setShowDoneSteps((v) => !v); if (isWork) setShowDoneWork((v) => !v); };
            return (
              <td key={i} style={{ background: "#FAF9FC", borderColor: C.border, color: C.faint }}
                onContextMenu={(isSteps || isWork) ? (e) => { e.preventDefault(); toggleDone(); } : undefined}
                title={(isSteps || isWork) ? "우클릭: 완료 항목 보기/숨기기" : undefined}
                className={`relative text-[10px] font-bold uppercase tracking-wide px-4 py-1.5 border-b text-center ${i > 0 ? "border-l" : ""}`}>
                {c}
                {(isSteps || isWork) && doneN > 0 && (
                  <span style={{ color: showing ? C.violet : C.faint }} className="ml-1 normal-case">
                    · 완료 {doneN}{showing ? " 표시" : ""}
                  </span>
                )}
      ...
      {!collapsed && (
        <tr style={{ opacity: dim ? 0.6 : 1 }} className="block md:table-row">
          {/* Current Status — stacked recent entries */}
          <td style={border} className={td}>
            <MobileLabel>Current Status</MobileLabel>
            <EntryStack entries={entries} field="currentStatus" />
          </td>

          {/* Next Step — follow-ups checklist */}
          <td style={border} className={`${td} md:border-l`}>
            <MobileLabel onClick={() => setShowDoneSteps((v) => !v)}
              hint={showDoneSteps ? "완료 숨기기" : "완료 보기"}>Next Step</MobileLabel>
            <Checklist items={p.nextSteps || []} readOnly={readOnly} onChange={setSteps}
              allowAdd={false} showDone={showDoneSteps} placeholder="F/U 추가…" emptyHint="—" />
          </td>

          {/* Brett Feedback — stacked (quotes + directives) */}
          <td style={border} className={`${td} md:border-l`}>
            <MobileLabel>Brett Feedback</MobileLabel>
            <EntryStack entries={entries} field="brett" />
          </td>

          {/* Other Feedback — stacked */}
          <td style={border} className={`${td} md:border-l`}>
            <MobileLabel>Other Feedback</MobileLabel>
            <EntryStack entries={entries} field="otherFeedback" />
          </td>

          {/* My Work — checklist */}
          <td style={border} className={`${td} md:border-l`}>
            <MobileLabel onClick={() => setShowDoneWork((v) => !v)}
              hint={showDoneWork ? "완료 숨기기" : "완료 보기"}>My Work</MobileLabel>
            <Checklist items={p.myWork || []} readOnly={readOnly} onChange={setWork}
              showDone={showDoneWork} placeholder="할 일 추가…" emptyHint="—" />
          </td>
        </tr>
      )}
      ...
      {editing && !collapsed && !readOnly && (
        <tr className="block md:table-row">
          <td colSpan={TCOLS.length} style={{ ...border, background: C.paper }} className="px-4 py-4 border-b block md:table-cell">
            <div style={{ color: C.faint }} className="text-[11px] mb-3">
              여기서 프로젝트 속성(제목 줄)과 미팅별 코멘트를 모두 편집할 수 있어요. Next Step·My Work는 표에서 직접 손봐요.
            </div>

            {/* Project name (rename) + merge into an existing project */}
            <div className="grid sm:grid-cols-2 gap-3 mb-3 pb-3" style={{ borderBottom: `1px solid ${C.border}` }}>
              <EditText label="프로젝트 이름" value={draft.name} onChange={(v) => setDraft({ ...draft, name: v })} />
              <div>
                <EditLabel>다른 프로젝트로 합치기</EditLabel>
                <select value={mergeTarget} onChange={(e) => setMergeTarget(e.target.value)}
                  style={border} className="w-full text-[13px] rounded-lg border px-2 py-1.5 bg-white">
                  <option value="">— 합치지 않음 —</option>
                  {(projectsAll || []).filter((x) => x.id !== p.id).map((x) => (
                    <option key={x.id} value={x.id}>{x.name}</option>
                  ))}
                </select>
                {mergeTarget && (() => {
                  const tgt = (projectsAll || []).find((x) => x.id === mergeTarget);
                  const nE = (p.entries || []).length, nS = (p.nextSteps || []).length, nW = (p.myWork || []).length;
                  return (
                    <div style={{ background: "#FCEBEF" }} className="mt-2 rounded-lg px-3 py-2 text-[11px]" >
                      <div style={{ color: "#B01238" }} className="mb-1.5">
                        이 프로젝트의 미팅 {nE}·Next {nS}·My Work {nW}개가 <b>{tgt ? tgt.name : ""}</b>로 합쳐지고,
                        <b> {p.name}</b>는 삭제돼요.
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => onMerge(mergeTarget)}
                          style={{ background: "#E11D48" }} className="text-white font-medium px-2.5 py-1 rounded-md">합치기 실행</button>
                        <button onClick={() => setMergeTarget("")} style={{ color: C.muted }} className="px-2 py-1">취소</button>
                      </div>
                    </div>
                  );
                })()}
            ...
            {/* edit every meeting's comment fields (newest → oldest) */}
            {draft.entries && draft.entries.length > 0 && (
              <div className="mt-4">
                <EditLabel>미팅별 코멘트 (최신 → 과거)</EditLabel>
                <div className="space-y-3 mt-1.5">
                  {[...draft.entries]
                    .sort((a, b) => (b.date || "").localeCompare(a.date || ""))
                    .map((e) => (
                      <div key={e.id} style={border} className="rounded-lg border p-3">
                        <div className="flex items-center gap-2 mb-2">
                          <MeetingTag mtype={e.mtype} />
                          <span style={{ color: C.faint }} className="text-[11px] font-mono">{e.date}</span>
                        </div>
                        <div className="grid md:grid-cols-3 gap-3">
                          <EditArea label="Current Status" value={e.currentStatus}
                            onChange={(v) => updateEntry(e.id, { currentStatus: v })} />
                          <EditArea label="Brett Feedback (· 구분, 시각화는 **볼드**)" value={e.brettFeedback}
                            onChange={(v) => updateEntry(e.id, { brettFeedback: v })} />
                          <EditArea label="Other Feedback (Trish·DS 등)" value={e.otherFeedback}
                            onChange={(v) => updateEntry(e.id, { otherFeedback: v })} />
                        </div>
                      </div>
                    ))}
                </div>
...
function Checklist({ items, readOnly, onChange, placeholder = "추가…", emptyHint = "—", allowAdd = true, showDone = false }) {
  ...
  // long-press (touch) opens the same highlight menu as right-click
  const lpRef = useRef({ timer: null, x: 0, y: 0 });
  const lpStart = (id, e) => {
    if (readOnly) return;
    const t = e.touches && e.touches[0]; if (!t) return;
    lpRef.current.x = t.clientX; lpRef.current.y = t.clientY;
    clearTimeout(lpRef.current.timer);
    lpRef.current.timer = setTimeout(() => {
      setMenu({ id, x: lpRef.current.x, y: lpRef.current.y });
      if (navigator.vibrate) navigator.vibrate(10);
    }, 500);
  };
  const lpMove = (e) => {
    const t = e.touches && e.touches[0]; if (!t) return;
    // cancel if the finger drifts (user is scrolling)
    if (Math.abs(t.clientX - lpRef.current.x) > 8 || Math.abs(t.clientY - lpRef.current.y) > 8) clearTimeout(lpRef.current.timer);
  };
  const lpEnd = () => clearTimeout(lpRef.current.timer);
  const setHl = (id, hl) => { onChange(items.map((it) => it.id === id ? { ...it, hl } : it)); setMenu(null); };

  // undone first, done after (each keeps insertion order)
  const ordered = showDone
    ...
    <div>
      ...
      <ul className="space-y-1">
        {ordered.map((it) => {
          ...
          <li key={it.id}
            ...
            onTouchStart={(e) => lpStart(it.id, e)}
            onTouchMove={lpMove}
            onTouchEnd={lpEnd}
            onTouchCancel={lpEnd}
            style={{
            ...
            className="group flex items-start gap-1 rounded px-1 -mx-1 select-none md:select-auto">
            {canDrag && (
              <span
                draggable
      ...
      {menu && (
        <>
          ...
          <div style={{
              left: Math.max(8, Math.min(menu.x, (typeof window !== "undefined" ? window.innerWidth : 9999) - 170)),
              top: Math.max(8, Math.min(menu.y, (typeof window !== "undefined" ? window.innerHeight : 9999) - 60)),
...
function EntryStack({ entries, field }) {
  const blocks = entries  // newest on top (entriesFor already sorts newest-first)
...
/* ------------------------------------------------------------------ */
function NotesView({ notes, projects, onDelete, onToggleFU, emptyHint }) {
      ...
      <div className="max-w-[1100px] w-full mx-auto px-6 sm:px-10 py-5">
        ...
        {shown.length === 0 ? (
          <div style={{ color: C.faint }} className="text-sm py-16 text-center">
            {emptyHint || "아직 노트가 없어요. 우측 상단 “노트 추가”로 참고 미팅 노트를 올려보세요."}
          </div>
        ) : (
...
/* ------------------------------------------------------------------ */
/*  Minimal Markdown renderer (headings, bold, bullets, tables, rules)  */
/* ------------------------------------------------------------------ */
function mdInline(s) {
  // **bold** and `code`
  const parts = [];
  ...
  while ((m = re.exec(s))) {
    ...
    const tok = m[0];
    if (tok.startsWith("**")) parts.push(<strong key={m.index} style={{ color: C.ink }}>{tok.slice(2, -2)}</strong>);
    else parts.push(<code key={m.index} style={{ background: C.section }} className="px-1 rounded text-[12px]">{tok.slice(1, -1)}</code>);
...
function Markdown({ text }) {
  ...
  while (i < lines.length) {
        ...
        <div key={k++} className="overflow-x-auto my-2">
          <table className="text-[12.5px] border-collapse w-full">
            <thead><tr>{head.map((h, j) => (
              <th key={j} style={{ background: C.section, borderColor: C.border, color: C.muted }}
                className="text-left font-semibold px-2.5 py-1.5 border">{mdInline(h)}</th>))}</tr></thead>
            <tbody>{body.map((r, ri) => (
              <tr key={ri}>{r.map((c, j) => (
                <td key={j} style={{ borderColor: C.border, color: C.ink }} className="px-2.5 py-1.5 border align-top">{mdInline(c)}</td>))}</tr>))}</tbody>
          </table>
        </div>);
      continue;
    }
    // horizontal rule
    if (/^\s*-{3,}\s*$/.test(l)) { out.push(<hr key={k++} style={{ borderColor: C.border }} className="my-3" />); i++; continue; }
    // headings
    const h = /^(#{1,4})\s+(.*)$/.exec(l);
    if (h) {
      const lvl = h[1].length;
      const cls = lvl === 1 ? "text-[16px] font-bold mt-1 mb-2"
                : lvl === 2 ? "text-[14px] font-bold mt-4 mb-1.5"
                : "text-[13px] font-semibold mt-3 mb-1";
      out.push(<div key={k++} style={{ color: lvl <= 2 ? C.violet : C.ink }} className={cls}>{mdInline(h[2])}</div>);
      i++; continue;
    }
    // bullet list
    if (/^\s*[-*]\s+/.test(l)) {
      ...
      out.push(
        <ul key={k++} className="space-y-1 my-1.5">
          {items.map((it, j) => (
            <li key={j} style={{ color: C.ink }} className="text-[13px] leading-relaxed flex gap-1.5">
              <span style={{ color: C.faint }} className="select-none">•</span><span className="flex-1">{mdInline(it)}</span>
            </li>))}
        </ul>);
      continue;
    }
    ...
    if (/^\s*\d+[.)]\s+/.test(l)) {
      ...
      out.push(
        <ol key={k++} className="space-y-1 my-1.5 list-decimal pl-5">
          {items.map((it, j) => <li key={j} style={{ color: C.ink }} className="text-[13px] leading-relaxed">{mdInline(it)}</li>)}
        </ol>);
      continue;
    }
    // paragraph (merge consecutive plain lines)
    const para = [];
    while (i < lines.length && lines[i].trim() && !isTableRow(lines[i]) && !/^(#{1,4})\s+/.test(lines[i])
           && !/^\s*[-*]\s+/.test(lines[i]) && !/^\s*\d+[.)]\s+/.test(lines[i]) && !/^\s*-{3,}\s*$/.test(lines[i])) {
      para.push(lines[i]); i++;
    }
    out.push(<p key={k++} style={{ color: C.ink }} className="text-[13px] leading-relaxed my-1.5">{mdInline(para.join(" "))}</p>);
  }
  return <div>{out}</div>;
}
...
function NoteCard({ n, projName, onDelete, onToggleFU }) {
  ...
  const hasTx = !!(n.transcript && n.transcript.trim());
  const hasReport = !!(n.report && n.report.trim());
  return (
    <div style={{ background: C.card, borderColor: C.border }}
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {hasTx && (
            <button onClick={() => setShowTx(true)} title="미팅 원문 보기">
          ...
          {confirmDel ? (
            ...
            <button onClick={() => setConfirmDel(true)} title="삭제">
      ...
      {showTx && (
        <TranscriptModal date={`${n.title || "노트"} · ${n.date}`} value={n.transcript}
          readOnly onClose={() => setShowTx(false)} onSave={() => setShowTx(false)} />
      )}
      ...
      {hasReport && (
        <div className="mt-3">
          <button onClick={() => setOpenReport((v) => !v)}
            style={{ background: C.violetSoft, color: C.violet, borderColor: "#A9D8C8" }}
            className="text-[12px] font-semibold px-3 py-1.5 rounded-lg border inline-flex items-center gap-1.5 hover:opacity-90 transition">
            {openReport ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
            {openReport ? "미팅리포트 접기" : "미팅리포트 펼치기"}
          </button>
          {openReport && (
            <div style={{ borderColor: C.border, background: C.paper }} className="mt-2 rounded-lg border px-4 py-3">
              <Markdown text={n.report} />
            </div>
          )}
        </div>
      )}

      {!hasReport && n.key && n.key !== "—" && (
        <div className="mt-3">
...
`이 회의 녹취를 Project Tracker의 미팅리포트 JSON으로 정리해줘. 코드블록 없이 JSON만 출력해줘.
  ...
  "notes": [
    {
      ...
      "transcript": "미팅 한글 번역본 전문 (더듬·반복만 제거, 내용은 그대로). 카드에서 원문으로 볼 수 있음. 없으면 생략",
      ...
      "report": "정식 미팅 리포트 전문(Markdown). 제목/요약/논의 내용/결정된 사항/다음 액션 표/오픈 이슈 순. 이 필드가 리포트 탭의 본문"
    }
  ]
}
...
function AddNote({ projects, onClose, onApply, buildNote }) {
  const [text, setText] = useState("");
  ...
  const build = () => preview.map(buildNote);
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6"
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div className="flex items-center justify-between px-5 py-4 sticky top-0"
          ...
          <h3 className="text-base font-semibold">미팅리포트 추가 (JSON)</h3>
          <IconBtn onClick={onClose} title="닫기"><X size={16} style={{ color: C.muted }} /></IconBtn>
        </div>
          ...
          {!preview ? (
            <>
              <div style={{ background: C.violetSoft, color: C.violet }}
                ...
                태그 없는 미팅(예: Copilot 팀 싱크)은 Claude에 녹취를 주고 “미팅리포트로 정리해줘”라고 하세요.
                받은 <code>.json</code> 파일을 올리면 돼요. (TT/INNO/DASH/1:1 미팅은 “미팅 추가”로 올리면 리포트가 자동으로 여기 쌓여요.)
...
function AddMeeting({ projects, onClose, onApply, isExcluded }) {
  const [text, setText] = useState("");
  const [fileName, setFileName] = useState("");
  ...
  const [revive, setRevive] = useState(() => new Set()); // preview row indexes the user chose to bring back
  const [error, setError] = useState("");
  const [copied, setCopied] = useState(false);
  const [preview, setPreview] = useState(null);
  ...
  const load = () => {
    ...
    setEffReports((data && Array.isArray(data.notes)) ? data.notes : []);
    setPreview(updates);
  };
          ...
          {preview && (
            <>
              <div style={{ color: C.muted }} className="text-xs flex items-center gap-2">
                ...
                {effReports && effReports.length > 0 && (
                  <span style={{ color: C.violet }} className="inline-flex items-center gap-1 ml-1">
                    · 미팅 리포트 {effReports.length}개 포함
                  </span>
                )}
              </div>
              <ul className="space-y-2">
                {preview.map((u, i) => {
                  ...
                  const ex = !match && isExcluded && isExcluded(u);
                  const willRevive = revive.has(i);
                  return (
                    <li key={i} style={{ borderColor: ex && !willRevive ? "#F3C7D0" : C.border, background: ex && !willRevive ? "#FDF5F7" : C.paper, opacity: ex && !willRevive ? 0.75 : 1 }}
                      className="rounded-lg border px-3 py-2.5">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="text-sm font-semibold">{match ? match.name : (u.name || "새 프로젝트")}</span>
                        {!match && !ex && <span style={{ background: C.violetSoft, color: C.violet }} className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full">NEW</span>}
                        {ex && (
                          <span style={{ background: "#FCEBEF", color: "#B01238" }} className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full">
                            삭제했던 프로젝트 · {willRevive ? "되살림" : "건너뜀"}
                          </span>
                        )}
                        {u.status && <span className="text-[10px]" style={{ color: C.faint }}>{STATUS[u.status]?.label || u.status}</span>}
                      </div>
                      {ex && (
                        <label className="mt-1.5 inline-flex items-center gap-1.5 text-[11px] cursor-pointer" style={{ color: C.violet }}>
                          <input type="checkbox" checked={willRevive}
                            onChange={(e) => setRevive((prev) => { const n = new Set(prev); e.target.checked ? n.add(i) : n.delete(i); return n; })} />
                          이 프로젝트 되살리기 (다시 추적)
                        </label>
                      )}
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => onApply(preview, effMtype, effDate, effTx, effReports, [...revive])}
                  style={{ background: C.violet }}
