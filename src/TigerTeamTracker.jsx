...
import {
  ...
  Loader2, AlertCircle, RotateCcw, Square, CheckSquare, GripVertical,
} from "lucide-react";
...
const DEFAULT_COLW = [16, 21, 26, 16, 21]; // Current / Next / Brett / Other / My Work
...
function TableRow({ p, entries, onSave, onDelete, readOnly, dim, collapsed, onToggleCollapse, colW, onColLive, onColCommit }) {
      ...
      <ul className="space-y-1">
        {ordered.map((it) => {
          ...
          <li key={it.id}
            ...
            style={overId === it.id ? { boxShadow: `inset 0 2px 0 ${C.violet}` } : undefined}
...
function AddMeeting({ projects, onClose, onApply }) {
  ...
  const fieldRules =
    ...
    "otherFeedback: feedback from anyone OTHER than Brett — Trish (manager), Josh/Philip and other data scientists, teammates. Prefix EACH point with the speaker's initial in square brackets, e.g. '[T] ...' for Trish, '[J] ...' for Josh, '[P] ...' for Philip; use '[?]' if the speaker is unclear. Empty if none. " +
            ...
            <>
              ...
              <div>
                ...
                <textarea ref={boxRef} value={text} onChange={(e) => setText(e.target.value)}
                  ...
                  style={{ borderColor: C.border }}
                  className="w-full mt-1 text-[13px] leading-relaxed rounded-lg border px-3 py-2.5 bg-white resize-y focus:outline-none focus:ring-2 focus:ring-teal-200" />
              </div>
              {error && (
