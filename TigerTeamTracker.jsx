...
export default function TigerTeamTracker() {
  ...
  const all = projects || [];
  const statusCount = (k) => all.filter((p) => p.status === k).length;
  const fuOpen = all.reduce((n, p) => n + (p.nextSteps || []).filter((i) => !i.done).length, 0);
  const workOpen = all.reduce((n, p) => n + (p.myWork || []).filter((i) => !i.done).length, 0);
    ...
    <div style={{ background: C.paper, color: C.ink }}
      ...
      <header className="max-w-[1600px] w-full mx-auto px-6 sm:px-10 pt-7 pb-4 shrink-0">
        <div className="flex items-end justify-between gap-4 flex-wrap">
          <div>
            <div style={{ color: C.faint, letterSpacing: "0.18em" }}
              className="text-[11px] font-semibold uppercase mb-1">
              Tiger Team · Project Log
            </div>
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="flex items-center gap-1.5 flex-wrap justify-end">
              ...
              <span style={{ color: C.border }} className="mx-0.5">|</span>
              <button onClick={() => setIncompleteOnly((v) => !v)}
                style={{
                  background: incompleteOnly ? C.violet : "#FFF",
                  color: incompleteOnly ? "#FFF" : C.faint,
                  borderColor: incompleteOnly ? C.violet : C.border,
                }}
      ...
      <div style={{ borderColor: C.border, background: "#fff" }}
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <Stat label="블로커" n={statusCount("blocked")} tone={STATUS.blocked}
            onClick={() => setGroupBy("status")} />
          <Stat label="리뷰" n={statusCount("review")} tone={STATUS.review}
            onClick={() => setGroupBy("status")} />
          <Stat label="진행중" n={statusCount("in-progress")} tone={STATUS["in-progress"]}
            onClick={() => setGroupBy("status")} />
          <Stat label="완료" n={statusCount("done")} tone={STATUS.done}
            onClick={() => setGroupBy("status")} />
          <span style={{ color: C.border }}>|</span>
          <Stat label="미완료 F/U" n={fuOpen} tone={{ bg: C.violetSoft, fg: C.violet }}
            onClick={() => setIncompleteOnly(true)} />
          <Stat label="미완료 할일" n={workOpen} tone={{ bg: C.violetSoft, fg: C.violet }}
            onClick={() => setIncompleteOnly(true)} />
        </div>
      ...
      <main className="flex-1 min-h-0 overflow-auto">
        <div style={{ borderColor: C.border }} className="border-y bg-white w-full">
          <table className="border-separate border-spacing-0 text-left w-full table-fixed">
            ...
            <thead>
              <tr>
                {TCOLS.map((c, i) => (
                  <th key={i} style={{ background: "#F7F5FB", borderColor: C.border, color: C.muted }}
                    className="sticky top-0 z-20 text-[11px] font-bold px-4 py-2.5 border-b text-center">{c}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {items.map((it) =>
                it.kind === "group" ? (
                  <GroupHeader key={"g:" + it.key} label={it.label} count={it.count} color={it.color} />
                ) : (
                  <TableRow key={it.p.id} p={it.p} entries={entriesFor(it.p)} readOnly={isHistory}
                    dim={it.p.status === "done"} incompleteOnly={incompleteOnly}
        ...
        <div className="px-3 sm:px-6">
          ...
          <p style={{ color: C.faint }} className="text-[11px] mt-3">
            프로젝트 이름을 누르면 접혀요 · Current Status / Brett / Other 는 최근 {VISIBLE_ENTRIES}개 미팅이 위에서부터 쌓여요 · Next Step(F/U)·My Work 는 체크박스로 완료 표시, 항목 클릭해 수정.
          </p>
        </div>
      </main>
...
function Stat({ label, n, tone, onClick }) {
  const dim = n === 0;
  return (
    <button onClick={onClick} disabled={!onClick}
      style={{ background: tone.bg, color: tone.fg, opacity: dim ? 0.45 : 1 }}
      className="text-[11px] font-semibold px-2 py-1 rounded-md inline-flex items-center gap-1.5
        hover:opacity-80 transition disabled:cursor-default">
      <span>{label}</span>
...
const TWIDTHS = ["3%", "18%", "17%", "27%", "18%", "17%"];

function TableRow({ p, entries, onSave, onDelete, readOnly, dim, incompleteOnly }) {
  ...
  return (
    <>
      ...
      <tr>
        <td colSpan={TCOLS.length} onClick={() => setCollapsed((v) => !v)}
          ...
          <div className="flex items-center justify-between gap-3 px-4 py-2" style={{ opacity: dim ? 0.6 : 1 }}>
            <div className="inline-flex items-center gap-2 flex-wrap min-w-0">
              ...
              {p.page && p.page !== "—" && (
                <span style={{ background: "#fff", color: C.violet, borderColor: "#D9CDFB" }}
                  className="text-[10px] font-semibold px-1.5 py-0.5 rounded border">📍 {p.page}</span>
              )}
              ...
              {p.lead && <span style={{ color: C.muted }} className="text-[11px]">{p.lead}</span>}
            </div>
            {!readOnly && (
          ...
          {!collapsed && p.location && p.location !== "—" && p.location !== p.page && (
            <div style={{ color: C.muted }} className="text-[11px] px-4 pb-2 -mt-1 pl-9">{p.location}</div>
          )}
        </td>
      </tr>
      ...
      {!collapsed && (
        <tr style={{ opacity: dim ? 0.6 : 1 }}>
          <td style={border} className={`${td} border-b`}></td>
          ...
          <td style={border} className={td}>
            <Checklist items={p.nextSteps || []} readOnly={readOnly} onChange={setSteps}
              incompleteOnly={incompleteOnly} placeholder="F/U 추가…" emptyHint="—" />
          </td>
          ...
          <td style={border} className={td}>
            <Checklist items={p.myWork || []} readOnly={readOnly} onChange={setWork}
              incompleteOnly={incompleteOnly} placeholder="할 일 추가…" emptyHint="—" />
          </td>
        </tr>
      )}
...
function Checklist({ items, readOnly, onChange, incompleteOnly, placeholder = "추가…", emptyHint = "—" }) {
  ...
  // undone first, done after (each keeps insertion order); hide done when focusing incomplete
  const visible = incompleteOnly ? items.filter((i) => !i.done) : items;
  const ordered = [...visible.filter((i) => !i.done), ...visible.filter((i) => i.done)];
  ...
  return (
    <div>
      ...
      <ul className="space-y-1">
        ...
        {ordered.length === 0 && (
          <li style={{ color: C.faint }} className="text-[12px]">
            {items.length > 0 && incompleteOnly ? "✓ 모두 완료" : emptyHint}
          </li>
        )}
      </ul>
    </div>
...
function AddMeeting({ projects, onClose, onApply }) {
  ...
  const parse = async () => {
    ...
    const sys =
      "You extract per-project status updates from a meeting transcript for a data-viz owner's project tracker. " +
      `This is a "${mtypeLabel}" meeting. ` +
      "The transcript is raw speech-to-text and may be garbled; infer intended names/terms from context. " +
      "Return ONLY valid JSON, no markdown, no prose. Schema: " +
      '{"updates":[{"id":"","name":"","status":"in-progress|blocked|review|on-track|done",' +
      ...
      "Use the existing project id when the discussion maps to one; leave id empty and set name for a genuinely new project. " +
      "Only include projects actually discussed. Leave a field empty if the meeting gives no update for it (do not invent). " +
      "page: a SHORT label (<=20 chars) for where this project's visualization lives (e.g. '분석 우측 패널', 'Task Analysis'); keep consistent across projects on the same page. Leave empty to keep the existing value. " +
      "location: a longer one-line description of the exact placement, if mentioned. Leave empty to keep existing. " +
      "vizPriority: 'high' | 'med' | 'low' — how central visualization/panel work is. Leave empty to keep existing. " +
      "currentStatus: brief status of where the project stands right now, as reported in the meeting. " +
      ...
      "otherFeedback: feedback from anyone OTHER than Brett — e.g. Trish (manager), Josh/Philip and other data scientists, teammates. Empty if none. " +
      "followUps: things the viz owner must FOLLOW UP ON or GET FROM other people to unblock their work — e.g. data/answers/artifacts owed by Josh, Philip, Trish, product, etc. Write each as 'F/U <person>: <what>' (e.g. 'F/U Josh: 절감 초 계산 데이터'). Empty if none. " +
      "nextWork: the viz owner's OWN concrete build steps (panels, views, layout, data wiring, design). Do NOT put follow-ups on other people here — those go in followUps. Empty if none. " +
      "For currentStatus, brettFeedback, otherFeedback, followUps, nextWork: return concise points separated by ' · ' (space, middle-dot, space). " +
      "In brettFeedback and otherFeedback, wrap any point about the VISUALIZATION/dashboard/panel/UI/what-to-display in **double asterisks**; leave non-visualization points unwrapped. " +
      "targetDate: keep very short — just a date or brief phrase (e.g. '8/31', '이번 주', '월말'), never a full sentence. " +
    ...
    try {
      ...
      if (!parsed.updates || !parsed.updates.length) throw new Error("empty");
      setPreview(parsed.updates);
    } catch (e) {
      setError("녹취를 해석하지 못했어요. 녹취가 너무 짧거나 형식 문제일 수 있어요 — 다시 시도하거나 내용을 조금 더 붙여넣어 주세요.");
    } finally {
      setLoading(false);
    }
  };
          ...
          {!preview && (
            <>
              ...
              <button onClick={parse} disabled={loading || text.trim().length < 40}
                ...
                {loading ? <><Loader2 size={16} className="animate-spin" /> 분석 중…</> : <>프로젝트별 업데이트 추출</>}
              </button>
